...
impl Widget for &App {
    fn render(self, area: ratatui::prelude::Rect, buf: &mut ratatui::prelude::Buffer) {
        ...
        let groups_block_style = styles::groups_block(&theme, self.state.focus == Focus::Groups);
        let filter_block_style = styles::filter_block(&theme, self.state.focus == Focus::Filter);
        let results_block_style = styles::results_block(&theme, self.state.focus == Focus::Results);

        let groups_item_style = styles::group_item(&theme, self.state.focus == Focus::Groups);
        let groups_selected_style =
            styles::groups_selected(&theme, self.state.focus == Focus::Groups);

        let groups_border = styles::pane_border(&theme, self.state.focus == Focus::Groups);
        let filter_border = styles::pane_border(&theme, self.state.focus == Focus::Filter);
        let results_border = styles::pane_border(&theme, self.state.focus == Focus::Results);
        ...
        let header_right_text: String = format!(
            "Profile: {} | Region: {}",
            self.state.profile.as_str(),
            self.state.region.as_str(),
        );
        Line::from(self.state.app_title.as_str())
            .bold()
            .style(theme.header)
            .render(header[0], buf);
        ...
        let footer_left = if let Some(msg) = &self.state.status_message {
            msg.clone()
        } else if self.state.group_search_active {
            format!("Search groups: {}", self.state.group_search_input)
        } else {
        ...
        let footer_right = if self.state.tail_mode {
        ...
        let start = self.state.groups_scroll;
        let end = (start + visible_rows).min(self.state.groups.len());

        for (row, idx) in (start..end).enumerate() {
            let group = &self.state.groups[idx];

            let selected = idx == self.state.selected_group;
        ...
        if self.state.searching && self.state.lines.is_empty() {
            let dots = ".".repeat(self.state.dots);
        ...
        let field_style = |field: FilterField| {
            let active = self.state.focus == Focus::Filter && field == self.state.filter_field;
            styles::filter_field(&theme, active, active && self.state.editing)
        };

        let line = |label: &str, value: &str| format!("{label}: {value}");

        Line::from(line("Start", &self.state.filter_start))
        ...
        Line::from(line("End", &self.state.filter_end))
        ...
        Line::from(line("Query", &self.state.filter_query))
        ...
        if self.state.focus == Focus::Filter && self.state.editing && self.state.cursor_on {
            ...
            if self.state.filter_field != FilterField::Search {
                let label = match self.state.filter_field {
                ...
                let cursor_col = self.state.filter_cursor_pos.min(value_len);
        ...
        if self.state.save_filter_popup_open {
            ...
            let name_line = format!("{}", self.state.save_filter_name);
            Line::from(name_line)
        ...
        if self.state.load_filter_popup_open {
            ...
            let needed_height = (self.state.saved_filters.len() as u16 + 3).max(3);
            ...
            let mut y = inner.y;
            for (idx, f) in self.state.saved_filters.iter().enumerate() {
                if y >= inner.y + inner.height {
                    break;
                }

                let marker = if idx == self.state.load_filter_selected {
                    ">"
                } else {
                    " "
                };
                let line = format!("{marker} {}", f.name);
                let style = if idx == self.state.load_filter_selected {
...
mod ui_tests {
    ...
    fn draws_cursor_when_editing_active_text_field() {
        ...
        app.state.cursor_on = true;
        app.state.filter_field = FilterField::Query;
        app.state.filter_query = "abc".to_string();
    ...
    fn does_not_draw_cursor_on_search_button() {
        ...
        app.state.editing = true;
        app.state.cursor_on = true;
        app.state.filter_field = FilterField::Search;
    ...
    fn shows_searching_message_when_searching_and_no_lines() {
        ...
        app.state.dots = 3;
        app.state.lines.clear(); // must be empty to trigger the early-return path
    ...
    fn header_and_footer_use_current_theme() {
        ...
        app.state.theme = crate::ui::styles::Theme::light();
