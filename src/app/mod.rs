...
impl App {
    ...
    fn open_save_filter_popup(&mut self) {
        self.save_filter_name.clear();
        self.save_filter_popup_open = true;
    }

    fn open_load_filter_popup(&mut self) {
    ...
    fn handle_save_filter_popup_key(&mut self, code: KeyCode) {
        match code {
            KeyCode::Esc => {
                self.save_filter_popup_open = false;
            }
            KeyCode::Enter => {
                // Save current filter under the given name (in-memory only)
                if !self.save_filter_name.trim().is_empty() {
                    let name = self.save_filter_name.trim().to_string();
                    // Overwrite if a filter with the same name exists
    ...
    fn handle_load_filter_popup_key(&mut self, code: KeyCode) {
        if self.saved_filters.is_empty() {
